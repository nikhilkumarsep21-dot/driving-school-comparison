    ...
    return (
      <Container>
        <div className="flex min-h-[60vh] flex-col items-center justify-center py-16 text-center">
          ...
          >
            Start by adding branches from the homepage to see a side-by-side comparison.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="text-lg text-gray-600">
              Side-by-side comparison of {branches.length} {branches.length === 1 ? 'branch' : 'branches'}
            </p>
          </div>
        </Container>
      </section>
              ...
              <table className="w-full border-collapse min-w-[800px] border-l border-gray-200">
                <thead>
                  <tr>
                    ...
                    {branches.map((branch, branchIndex) => (
                      <th key={branch.id} className="bg-white border-b border-r border-gray-200 p-6 min-w-[320px]">
                        <motion.div
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                        ...
                        >
                          ...
                          <div className="space-y-3">
                            ...
                            <p className="text-sm text-gray-600">{branch.school?.name}</p>
                            {branch.school && (
                              <div className="flex items-center justify-center gap-2">
                                <StarRating rating={branch.school.rating} size="sm" />
                                <span className="text-sm text-gray-600">
                                  {branch.school.rating} ({branch.school.review_count})
                                </span>
                              </div>
                            )}
                ...
                <tbody>
                  <ComparisonRowData
                    ...
                    renderCell={(branch) => (
                      <div className="text-center">
                        <p className="font-semibold text-gray-900">{branch.city || 'Dubai'}</p>
                  ...
                  <ComparisonRowData
                    ...
                    schools={branches}
                    renderCell={(branch) => (
                      <p className="text-center whitespace-pre-wrap font-medium text-gray-900 text-sm">{branch.normal_hours || 'Contact for hours'}</p>
                    )}
                  />
                  ...
                  <ComparisonRowData
                    ...
                    renderCell={(branch) => (
                      <div className="text-center">
                        <p className="font-semibold text-gray-900">{branch.contact || 'N/A'}</p>
                        {branch.email && (
                          <p className="mt-1 text-sm text-gray-500">{branch.email}</p>
                        )}
                      </div>
                    )}
                  />
                      ...
                      <div className="space-y-2">
                        ...
                        {branch.email && (
                          ...
                          >
                            ...
                            <span className="text-sm font-medium truncate">{branch.email}</span>
                          </a>
                        )}
